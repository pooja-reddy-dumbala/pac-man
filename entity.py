...
def translate_screen_to_coord_hero(in_coords, in_size=24):
    half_in_size = in_size // 2
    return int((in_coords[0] + half_in_size) / in_size), int((in_coords[1] + half_in_size) / in_size)

def translate_screen_to_coord(in_coords, in_size=24):
    return int((in_coords[0]) / in_size), int((in_coords[1]) / in_size)

def translate_to_screen(in_coords, in_size=24):
...
class GameEntity:
    def __init__(self, in_surface, x, y,
        ...
        self._surface = in_surface.screen
        self.y = y
        self.x = x
...
class Ghost(MovableEntity):
    ...
    def request_path_to_player(self, in_ghost):
        player_position = translate_screen_to_coord(in_ghost._renderer.get_hero_position())
